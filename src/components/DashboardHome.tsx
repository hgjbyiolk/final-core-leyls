...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
  {stats.map((stat, index) => {
    ...
    return (
      ...
      >
        {/* Always-on subtle brand tint */}
        <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-[#E6A85C]/5 via-[#E85A9B]/5 to-[#D946EF]/5 pointer-events-none" />
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-[11px] uppercase tracking-wide text-gray-400 mb-1">{stat.name}</p>
            <p className="text-3xl font-extrabold text-gray-900 tracking-tight">{stat.value}</p>
          </div>
        </div>

        {/* Trend pill + description */}
        <div className="relative mt-3 flex flex-col space-y-1">
          <span
          ...
          >
            {stat.change}
            {trendUp ? (
              <ArrowUpRight className="h-3 w-3 ml-1 stroke-[2.5]" />
            ) : (
              <ArrowDownRight className="h-3 w-3 ml-1 stroke-[2.5]" />
            )}
          </span>
          {stat.description && (
            <p className="text-[11px] text-gray-500">{stat.description}</p>
          )}
        </div>
      </div>
    );
